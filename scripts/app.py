...
if choice == "Overview":
    ...
    The primary goal of this application is to develop and evaluate deep learning models for classifying bird species based on their audio recordings.  Through PyTorch and audio processing techniques, we aim to create a system that can accurately identify bird species from short audio clips.
...
if choice == "Model Training":
    ...
    # Plot data from all event files with color differentiation
    def load_and_plot_all_runs_data(runs_dir):
        ...
        for idx, logdir in enumerate(run_paths):
            ...
            try:
                ...
                # Default learning rate for fixed learning rate events
                default_lr = [0.001] * len(train_steps)
...
if choice == "Model Inference":
    ...
    # Replace abbreviations with full terms for display name
    replacements = {
        'sch': 'Scheduled',
        'lr': 'Learning Rate',
        'es': ', Early Stopping',
    ...
    for model_name in model_directories:
        ...
        for abbr, full in replacements.items():
            display_name = display_name.replace(abbr, full)

        st.header(f"Metrics | {display_name.replace('_', ' ').title()}")

        # Original name
        metrics_path = f"{paths['results_dir']}/metrics/metrics_{model_name}.json"
        try:
            with open(metrics_path, "r") as f:
                metrics = json.load(f)
            st.write(f"Test Loss: {float(metrics['test_loss']):.4f} | Test Accuracy: {float(metrics['test_accuracy']):.2f}%")
        except FileNotFoundError:
            st.error(f"Model metrics file not found for {display_name}. Please ensure the path is correct.")

        confusion_matrix_path = f"{paths['results_dir']}/confusion_matrices/confusion_matrix_{model_name}.fig"
        try:
            ...
            st.error(f"Confusion matrix file not found for {display_name}. Please ensure the path is correct.")